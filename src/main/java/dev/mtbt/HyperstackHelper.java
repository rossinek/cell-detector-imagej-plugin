...
public class HyperstackHelper {
  public static ImagePlus extractFrame (ImagePlus imp, int channel, int slice, int frame) {
    return SubHyperstackMaker.makeSubhyperstack(imp, Integer.toString(channel), Integer.toString(slice), Integer.toString(frame)).duplicate();
  }
  ...
  public static ImagePlus extractGrayFrame (ImagePlus imp, int channel, int slice, int frame, int bitDepth) {
    ...
    switch (bitDepth) {
      ...
      case 32:
        ...
        break;
      default:
        throw new IllegalArgumentException("[HyperstackHelper] Can't find converter for provided input.");
    }

 ...
 *
 * This plugin reapplies the transformations logged by Image_Stabilizer to another stack.
 *
 * Authors: Kang Li (kangli AT cs.cmu.edu) Steven Kang (sskang AT andrew.cmu.edu)
 *
 * Requires: ImageJ 1.38q or later & JRE 5.0 or later
 *
 * Installation: Download Image_Stabilizer_Log_Applier.java to the plugins folder or its subfolder.
 * Compile and run it using Plugins/Compile and Run. Restart ImageJ and there will be a new "Image
 * Stabilizer Log Applier" command in the Plugins menu or its submenu.
 *
 * History: 2009/01/11: First version
 */
    ...
    for (int i = 0; i < fs.length; ++i) {
      if (fs[i] instanceof Editor) {
        ...
        if (!temp.startsWith("Image Stabilizer Log File"))
          ...
          IJ.error(
              "Multiple log windows are open. Please close all logs but the one you wish to apply.");
          return false;
        }

...
public class Graph {
  ...
  public Graph (sc.fiji.analyzeSkeleton.Graph g) {
    this.edges = new TreeSet<>();
    this.vertices = new TreeSet<>();
    final Map<sc.fiji.analyzeSkeleton.Vertex, Vertex> vertexMap = g.getVertices().stream().collect(toMap(identity(), Vertex::new));
    final Function<sc.fiji.analyzeSkeleton.Edge, Edge> cloner = e -> new Edge(e, vertexMap.get(e.getV1()), vertexMap.get(e.getV2()));
    final Map<sc.fiji.analyzeSkeleton.Edge, Edge> edgeMap = g.getEdges().stream().collect(toMap(identity(), cloner));
	...
	public Edge addEdge (Edge e) {
	  Edge newEdge = e;
	  if (!this.edges.add(e)) newEdge = this.edges.floor(e);
    ...
    if (!this.vertices.add(v)) return this.vertices.floor(v);
    return v;
	}
  ...
  public Edge closestEdge (Point point) {
	...
	public Graph clone() {
		...
		final Map<Vertex, Vertex> vertexMap = vertices.stream().collect(toMap(identity(), Vertex::cloneUnconnected));
		final Function<Edge, Edge> cloner = e -> e.clone(vertexMap.get(e.getV1()), vertexMap.get(e.getV2()));

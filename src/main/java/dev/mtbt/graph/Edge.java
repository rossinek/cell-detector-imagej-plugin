...
public class Edge implements Comparable<Edge> {
  ...
  private void fixSlabsDirection () {
    if (this.slabs.isEmpty()) return;
    if (distance(this.v1.center(), slabs.get(0)) > distance(this.v1.center(), slabs.get(slabs.size() - 1))) {
  ...
  public ArrayList<Point> getDirectedSlabs (Vertex begin) {
    if (!begin.equals(this.v1) && !begin.equals(this.v2)) throw new IllegalArgumentException("Begin is not edge vertex");
    ArrayList<Point> slabs = new ArrayList<>(this.slabs);
    if (begin.equals(this.v2)) Collections.reverse(slabs);
    return slabs;
  }

  public Vertex getOppositeVertex (Vertex v) {
    if (this.v1.equals(v)) return this.v2;
    else if (this.v2.equals(v)) return this.v1;
    return null;
  }
  ...
  public Edge clone (final Vertex v1, final Vertex v2) {
    ...
    if (slabs != null) {
      clonedSlabs = slabs.stream().map(Point::clone).collect(Collectors.toCollection(ArrayList::new));
    }
